...
class Page(object):
    ...
    def __init__(self,page_num,total_count,per_page=10,max_page=11):
        ...
        self.start = (self.page_num-1)*self.per_page
    ...
    def page_html(self,url_prefix):
        ...
        '''
        # 获取最大页数
        pages = math.ceil(self.total_count / self.per_page)
        # 分页器的中间页数
        ...
        for i in range(page_start, page_end + 1):
            ...
            if i == self.page_num:
                html_str = '''<li class='active'><a href="{url}?page={0}&pageNumber={1}">{0}</a></li>'''.format(i,self.per_page,url=url_prefix)
            else:
                html_str = '''
                 <li><a href="{url}?page={0}&pageNumber={1}">{0}</a></li>'''.format(i, self.per_page,url=url_prefix)
            pager_list.append(html_str)

        # 返回上一页的li标签
        pre_page_html = '''<li class='previous'><a href="{url}?page={0}&pageNumber={1}"><span>&laquo;</span></a></li>'''.format(
            self.page_num - 1, self.per_page,url=url_prefix)

        # 如果当前页是第一页，上一页按钮不可点击
        if self.page_num == 1:
            pre_page_html = '''<li class="previous disabled"><a href="{url}?page={0}&pageNumber={1}"><span>&laquo;</span></a></li>'''.format(
                self.page_num - 1, self.per_page,url=url_prefix)
        pager_list.insert(0, pre_page_html)

        # 返回第一页的li标签
        first_page_html = '''<li><a href="{url}?page={0}&pageNumber={1}">首页</a></li>'''.format(1, self.per_page,url=url_prefix)
        pager_list.insert(0, first_page_html)
        # 下一页
        ...
        # # 访问最后一页的li标签
        last_page_html = '''<li><a href="{url}?page={0}&pageNumber={1}">尾页</a></li>'''.format(pages, self.per_page,url=url_prefix)
